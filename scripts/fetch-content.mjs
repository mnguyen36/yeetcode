// Fetches question descriptions (HTML) for every free problem that has at
// least one local solution file. Resumable: skips slugs already downloaded
// into data/raw/content/<slug>.json.
...
const OUT_DIR = join(ROOT, "data", "raw", "content");
...
async function fetchContent(slug, attempt = 1) {
  ...
  if (!res.ok) {
    if (attempt <= 3) {
      ...
      return fetchContent(slug, attempt + 1);
    }
...
// Only fetch descriptions for problems we actually have solutions for.
...
console.log(`${targets.length} free problems with solutions`);
...
for (const p of targets) {
  ...
  try {
    const q = await fetchContent(p.titleSlug);
    writeFileSync(outFile, JSON.stringify({ slug: p.titleSlug, ...q }, null, 1));
    done++;
    if (done % 25 === 0) console.log(`downloaded ${done} (skipped ${skipped})`);
  } catch (e) {
    failed++;
